...
# M4K: controller katmanı oluşturuldu; GUI'nin business logic'e doğrudan erişimi kaldırıldı
import os
import threading
...
# M4K: profil dosyası yolu config'den okunuyor; varsayılan ~/.config/gpu-switcher/profiles.yaml
...
class GpuProfile:
    ...
    # M4K: profil dataclass'ı oluşturuldu; YAML ile seri/deseri için asdict() uyumlu
    name: str
...
class ApplySettings:
    ...
    # M4K: view'den controller'a aktarılan ayar nesnesi; doğrudan widget state tutulmuyor
    prime_mode: str
    comp_pipeline: bool
    full_rgb: bool
    autostart: bool
...
class GpuController:
    ...
    def __init__(self):
        # M4K: başlangıçta sistem durumu algılanıyor; view init'ten önce hazır
    ...
    def detect_display(self) -> str:
        ...
        # M4K: state cache güncelleniyor; view set_subtitle ile yansıtır
    ...
    def apply_async(
        ...
        """
        # M4K: apply işlemi daemon thread'e taşındı; apply sırasında UI donmuyor
    ...
    def _run_apply(
        ...
        # M4K: tüm core çağrıları bu thread içinde; hata yakalanıp on_done(False) ile bildirilir
        success = True
        try:
    ...
    def save_profile(self, profile: GpuProfile) -> bool:
        ...
        # M4K: aynı isimli profil varsa replace edilir; sıralama korunur
    ...
    def _load_profiles(self) -> None:
        # M4K: PyYAML yoksa veya dosya bozuksa profiller boş başlar; uygulama çökmez
        try:
            import yaml
    ...
    def _persist_profiles(self) -> bool:
        # M4K: dizin yoksa oluşturuluyor; yazma hatası False döndürür, uygulama çökmez
        try:
            import yaml
